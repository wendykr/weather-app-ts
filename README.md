# Aplikace na zobrazení počasí / verze React + Typescript
...
![](zadani/weather_app_starter.jpg)
...
Je možné, že základní teplota, kterou open weather posílá bude v kelvinech, v takovém případě přidej do URL parametr `units=metric`. 
Tvoje API URL by mohlo vypadat nějak takhle:       
`api.openweathermap.org/data/2.5/weather?q=Prague&units=metric&appid={tve unikátní API ID}`     
...
V `App.js` si vytvoř stav, do kterého budeš ukládat data získaná z API. Vhodně si ho nazvi, třeba `weather` a funkci na změnu stavu `setWeather`. Jako defaultní hodnotu použij `null`.
...
![](zadani/current_weather_data.jpg) 
...
Pozor, abys na onClick funkci pouze předávala a nevolala ji! Předávat funkci s parametrem můžeš pomocí anonymní funkce:     
...
![](zadani/fetch_forecast_object.jpg)
...
Při fetchování dat o předpovědi místo výpisu do konzole nastav stav `forecast`. Není třeba si ukládat celý objekt s polem o 40 položkách, proto při volání `setForecast` použij svou filtrovací funkci a tak nastavíš do stavu pouze data, která budeme používat. 

#### Příprava dat
Po provedení fetche budeš mít v proměnné `forecast` pole s položkami takového formátu: 

![](zadani/forecast_array_item.jpg)
...
Pokud `forecast` není `undefined` nebo `null`, zobraz předpověď. V opačném případě nezobrazuj nic, anebo komponentu Loading, kterou sis vytvořila. 
#### Komponenta
...
## Úklid kódu 
Asi se ti ted v App.js a na různých místech nahromadily všemožné funkce a pomocná pole. Trochu si kód uklidíme. V `src` si vytvoř složku `utils`. V ní si vytvoř javascriptový soubor, do kterého přesuneš všechny pomocné funkce - převod unixtimestamp na čas a na datum, filtrování pole, případně pomocná pole s dny v týdnu a měsíci. Tyto funkce si exportuj a pouze je importuj na místech, kde je využíváš.     
...
Níže vidíš připravený HTML kód, který si můžeš překopírovat do aplikace. 
```html
